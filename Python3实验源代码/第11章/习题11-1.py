...
def updateRecent(): 
    ...
    f.close()
    mfile.delete(2)                                     #删除原来的“最近”菜单项    
    recent=Menu(tearoff=0)
    mfile.insert_cascade(2,label='最近',menu=recent)    #重建“最近”菜单项
    n=1    
    for c in fs:
        c=c.strip()
...
'''
insertRecord()：为表格添加一个空行，用于添加新记录
'''
def insertRecord():
    columns = ('新记录','','','','','')
    if len(treeview.selection())<1:
        ...
        row = treeview.selection()[0]           #返回右击单元格所在的行名    
        ...
        if rn<0:
            rn=0
        print(rn)
    n=treeview.insert('',rn, values=columns) #插入新行
    treeview.selection_set(n)               #选中该行
    treeview.see(n)                         #使其可见
...
#============定义弹出菜单=====================
mpop=Menu(root,tearoff=0)
mpop.add_command(label='添加记录',command=insertRecord)
mpop.add_command(label='删除记录',command=delRecord)

#============实现记录查找功能=================
def findrecord():
  k=simpledialog.askstring('查找记录', '请输入要查找的姓名或者学号：')
  if k:
    for n in treeview.get_children():
       row=treeview.item(n,"values")
       if k in row[1] or k in row[0]:           
           treeview.selection_set(n)
           treeview.see(n)
           break
    else:
        showinfo('查找记录','未找到匹配的记录！')
mpop.add_separator()
mpop.add_command(label='查找记录',command=findrecord)
#============显示弹出菜单=================
def showpop(e):
   row = treeview.identify_row(e.y)
   treeview.selection_set(row)
   mpop.post(e.x_root,e.y_root)
treeview.bind('<Button-3>', showpop)
#==========================================
root.mainloop()
